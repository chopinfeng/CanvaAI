...
题目左边放原图、右边放转换后的画板内容，并排对照——**转换对不对，看一眼就知道**：

```bash
cd apps/server && npx tsx scripts/seed-exam-set.ts exam-set
```

已内置 **20 道自编题**（12 个考点、三档难度），每步答案都经过验算。
详见 [docs/PROBLEM-SETS.md](docs/PROBLEM-SETS.md)。
...
**撤销以"一次动作"为单位，AI 画的也能撤。** `⌘Z` / `Ctrl+Z`（重做 `⌘⇧Z` / `Ctrl+Y`）。
AI 画一座房子产生十几个图元，按一次就全没了。但**撤不掉别的协作者的内容**——
那需要把"AI 的改动"和"别人的改动"在客户端区分开，靠一个单独的同步帧标签实现。
