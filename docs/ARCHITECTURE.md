...
- **人不在就不动手**：页面切到后台、或窗口失去焦点时，闸门关闭——静止再久也不放行。
  内容继续攒着，等用户切回来**重新计时** 5 秒再说（刚回来画布就自己动起来同样吓人）。
  判定用 `visibilityState === 'visible' && document.hasFocus()`：只看前者拦不住
  "浏览器开着但用户在别的程序里"。
  闸门关着时面板显示「等你回来…」而不是「等你画完…」——不说清楚，用户会以为 AI 坏了。
