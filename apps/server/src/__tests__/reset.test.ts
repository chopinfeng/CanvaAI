...
describe('会话重置', () => {
  it('清掉辅导账本和模式，画布不动', async () => {
    ...
    room.session.tutor = {
      ...
      rightSince: 1, markedSinceAsk: false, attempts: []
    };
  ...
  it('重置之后不会有半路的辅导账本残留下来', async () => {
    ...
    room.session.mode = 'tutor';
    room.session.tutor = { goal: 'x', outline: [{ text: 'a', done: false }], startedTurn: 1, pending: null, rightSince: 0, markedSinceAsk: false, attempts: [] };
